...
class First_8_Marketing
{
    ...
    */
        ...
        */
        public function f8m_store_redirected_data()
        {
                if (isset($_GET['umamiScript']) && isset($_GET['websiteId'])) { // phpcs:ignore
                        ...
                        $website_id = sanitize_text_field(wp_unslash($_GET['websiteId']));
    ...
    public function f8m_enqueue_umami_script() {
        ...
        if (!empty($umami_script) && !empty($website_id)) {
            // Register the script with the external URL and load it in the footer
            wp_register_script(
                'umami-script',
            ...
            // Update the site option to indicate the script was injected
            ...
            add_filter('script_loader_tag', function($tag, $handle) use ($website_id) {
                // Check if this is the 'umami-script' handle
                if ('umami-script' === $handle) {
                    // Add the defer attribute and data-website-id attribute
                    return str_replace(' src', ' defer src', str_replace('></script>', ' data-website-id="' . esc_attr($website_id) . '"></script>', $tag));
                }
                return $tag;
            }, 10, 2);
        }
    ...
    */
    public function export_post() {
        $api_key = get_site_option('first_8_marketing_api_key');
        $website_id = get_option('umami_website_id'); // phpcs:ignore
        ...
        if ($api_key) {
            ...
            if ($query->have_posts()) {
                ...
                $response = wp_remote_post(FIRST_8_MARKETING_SCRAP_API_URL . '/api/bespoke/websites/'. $website_id .'/posts', array(
                    'method'    => 'POST',
